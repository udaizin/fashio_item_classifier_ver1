...
def predict(img):
    ...
    img = img.resize((img_size, img_size)) # サイズの変換
    ...
    # 結果を返す
    y_prob = torch.nn.functional.softmax(torch.squeeze(y)) # 確率で返す
    sorted_prob, sorted_indices = torch.sort(y_prob, descending=True) # 降順に
